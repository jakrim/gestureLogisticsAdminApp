  ...
  return async (dispatch) => {
    ...
    try {
      const response = await fetch(
        ...
        `${URL}/logistics_order?${_basicUrl}&orderId=${orderID}`
      );
        ...
        loadedOrder.push(
          new Order(
            order[key].address_coordinates
              ? order[key].address_coordinates
              : '',
            order[key].address_string ? order[key].address_string : '',
            order[key].address_string_2 ? order[key].address_string_2 : '',
            order[key].category_name ? order[key].category_name : '',
            order[key].delivery_note ? order[key].delivery_note : '',
            order[key].orderID ? order[key].orderID : '',
            order[key].os ? order[key].os : '',
            order[key].product_name ? order[key].product_name : '',
            order[key].recipient_email ? order[key].recipient_email : '',
            order[key].recipient_name ? order[key].recipient_name : '',
            order[key].recipient_phone_number
              ? order[key].recipient_phone_number
              : '',
            order[key].sender_email ? order[key].sender_email : '',
            order[key].sender_name ? order[key].sender_name : '',
            order[key].sender_phone_number
              ? order[key].sender_phone_number
              : '',
            order[key].time_order_placed ? order[key].time_order_placed : '',
            order[key].zone ? order[key].zone : ''
          )
        );
      }
